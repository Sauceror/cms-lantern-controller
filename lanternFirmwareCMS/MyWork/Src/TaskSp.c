...
#define SP1_TX_BUF_SIZE 64
static uint8_t sp1TxBuf[SP1_TX_BUF_SIZE];
static uint8_t *txBufHalf = &sp1TxBuf[SP1_TX_BUF_SIZE / 2];
...
{
	rxCmd.rxLen = 0;
	rxCmd.size = SP1_RX_BUF_SIZE;
	rxCmd.buffer = sp1RxBuf;
			...
			if (rxCmd.buffer[CMD_INDEX_SLVID] == SLV_ID || rxCmd.buffer[CMD_INDEX_SLVID] == 0xFF)
			{
				...
				for (int i = 0; i < sizeof(SLV_CMD) / sizeof(SLV_CMD[0]); i++)
				{
						...
						(SLV_CMD[i].cmd_len == 0 || SLV_CMD[i].cmd_len == rxCmd.rxLen))
					{
						...
						{
							len = Cmd_Encode(txBufHalf, sp1TxBuf, len);
							if (len > 0)
							{
								SpWrite(this_sp, sp1TxBuf, len);
							}
						}
						break;
					}
